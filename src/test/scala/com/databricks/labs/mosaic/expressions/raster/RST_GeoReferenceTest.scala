...
class RST_GeoReferenceTest extends QueryTest with SharedSparkSessionGDAL with RST_GeoReferenceBehaviors {
    ...
    test("Testing RST_GeoReference with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            geoReferenceBehavior(H3IndexSystem)
        }
    }
