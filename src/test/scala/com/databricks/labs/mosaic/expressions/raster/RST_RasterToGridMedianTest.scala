...
class RST_RasterToGridMedianTest extends QueryTest with SharedSparkSessionGDAL with RST_RasterToGridMedianBehaviors {
    ...
    test("Testing RST_RasterToGridMedian with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            rasterToGridMedianBehavior(H3IndexSystem)
        }
    }
