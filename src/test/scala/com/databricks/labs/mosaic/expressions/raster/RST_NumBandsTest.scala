...
class RST_NumBandsTest extends QueryTest with SharedSparkSessionGDAL with RST_NumBandsBehaviors {
    ...
    test("Testing RST_NumBands with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            numBandsBehavior(H3IndexSystem)
        }
    }
