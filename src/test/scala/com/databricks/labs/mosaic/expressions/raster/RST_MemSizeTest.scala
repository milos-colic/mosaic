...
class RST_MemSizeTest extends QueryTest with SharedSparkSessionGDAL with RST_MemSizeBehaviors {
    ...
    test("Testing RST_MemSize with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            memSizeBehavior(H3IndexSystem)
        }
    }
