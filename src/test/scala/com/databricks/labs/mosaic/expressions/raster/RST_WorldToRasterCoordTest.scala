...
class RST_WorldToRasterCoordTest extends QueryTest with SharedSparkSessionGDAL with RST_WorldToRasterCoordBehaviors {
    ...
    test("Testing RST_WorldToRasterCoord with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            worldToRasterCoordBehavior(H3IndexSystem)
        }
    }
