...
class RST_TessellateTest extends QueryTest with SharedSparkSessionGDAL with RST_TessellateBehaviors {
    ...
    test("Testing RST_GridTiles with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            tessellateBehavior(H3IndexSystem)
        }
    }
