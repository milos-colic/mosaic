...
class RST_PixelWidthTest extends QueryTest with SharedSparkSessionGDAL with RST_PixelWidthBehaviors {
    ...
    test("Testing RST_PixelWidth with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            pixelWidthBehavior(H3IndexSystem)
        }
    }
