...
class RST_SeparateBandsTest extends QueryTest with SharedSparkSessionGDAL with RST_SeparateBandsBehaviors {
    ...
    test("Testing RST_SeparateBands with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            separateBandsBehavior(H3IndexSystem)
        }
    }
