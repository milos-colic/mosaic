...
import com.databricks.labs.mosaic.expressions.SpatialSQLAPIsMock.{st_area, st_buffer, st_centroid, st_geomfromgeojson}
...
trait RST_ClipBehaviors extends QueryTest {
    ...
    def basicBehaviour(indexSystem: IndexSystem): Unit = {
    ...
    def cutlineBehaviour(indexSystem: IndexSystem): Unit = {
        spark.sparkContext.setLogLevel("ERROR")
        val mc = MosaicContext.build(indexSystem)
        mc.register()
        val sc = spark
        ...
        val clipRegion = testRaster
            .bbox(testRaster.getSpatialReference)
            .getCentroid
            .buffer(buffVal)
