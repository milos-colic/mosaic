...
trait CellUnionBehaviors extends MosaicSpatialQueryTest {
    ...
    def behaviorComputedColumns(mosaicContext: MosaicContext): Unit = {
        ...
        val res = leftDf
            ...
            .withColumn("union", grid_cell_union($"left_chip", $"right_chip"))
            .select($"case_id", st_aswkt($"union.wkb").alias("actual_wkt"))
            .join(expDf, "case_id")
    ...
    def columnFunctionSignatures(mosaicContext: MosaicContext): Unit = {
        ...
        noException should be thrownBy funcs.grid_cell_union(col("wkt"), col("wkt"))
    }
    ...
    def auxiliaryMethods(mosaicContext: MosaicContext): Unit = {
        ...
        noException should be thrownBy cellUnionExpr.makeCopy(cellUnionExpr.children.toArray)
        noException should be thrownBy cellUnionExpr.withNewChildrenInternal(Array(cellUnionExpr.children: _*))
    }
