...
class RST_RasterToGridMaxTest extends QueryTest with SharedSparkSessionGDAL with RST_RasterToGridMaxBehaviors {
    ...
    test("Testing RST_RasterToGridMax with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            rasterToGridMaxBehavior(H3IndexSystem)
        }
    }
