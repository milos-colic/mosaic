...
class RST_WidthTest extends QueryTest with SharedSparkSessionGDAL with RST_WidthBehaviors {
    ...
    test("Testing RST_Width with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            widthBehavior(H3IndexSystem)
        }
    }
