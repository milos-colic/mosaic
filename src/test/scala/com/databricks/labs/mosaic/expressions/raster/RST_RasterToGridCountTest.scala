...
class RST_RasterToGridCountTest extends QueryTest with SharedSparkSessionGDAL with RST_RasterToGridCountBehaviors {
    ...
    test("Testing RST_RasterToGridCount with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            rasterToGridCountBehavior(H3IndexSystem)
        }
    }
