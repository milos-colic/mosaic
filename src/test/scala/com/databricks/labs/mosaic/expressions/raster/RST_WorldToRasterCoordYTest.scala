...
class RST_WorldToRasterCoordYTest extends QueryTest with SharedSparkSessionGDAL with RST_WorldToRasterCoordYBehaviors {
    ...
    test("Testing RST_WorldToRasterCoordY with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            worldToRasterCoordYBehavior(H3IndexSystem)
        }
    }
