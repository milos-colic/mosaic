...
class RST_SummaryTest extends QueryTest with SharedSparkSessionGDAL with RST_SummaryBehaviors {
    ...
    test("Testing RST_Summary with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            summaryBehavior(H3IndexSystem)
        }
    }
