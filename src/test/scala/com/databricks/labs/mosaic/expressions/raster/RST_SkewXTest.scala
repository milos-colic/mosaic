...
class RST_SkewXTest extends QueryTest with SharedSparkSessionGDAL with RST_SkewXBehaviors {
    ...
    test("Testing RST_SkewX with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            skewXBehavior(H3IndexSystem)
        }
    }
