...
class RST_RasterToWorldCoordTest extends QueryTest with SharedSparkSessionGDAL with RST_RasterToWorldCoordBehaviors {
    ...
    test("Testing RST_RasterToWorldCoordTest with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            rasterToWorldCoordBehavior(H3IndexSystem)
        }
    }
