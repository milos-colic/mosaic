...
class RST_WorldToRasterCoordXTest extends QueryTest with SharedSparkSessionGDAL with RST_WorldToRasterCoordXBehaviors {
    ...
    test("Testing RST_WorldToRasterCoordX with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            worldToRasterCoordXBehavior(H3IndexSystem)
        }
    }
