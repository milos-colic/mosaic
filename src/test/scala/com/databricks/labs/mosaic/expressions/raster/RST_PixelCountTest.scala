...
class RST_PixelCountTest extends QueryTest with SharedSparkSessionGDAL with RST_PixelCountBehaviors {
    ...
    test("Testing rst_pixelcount behavior with H3IndexSystem and JTS") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            behavior(H3IndexSystem)
        }
    }
