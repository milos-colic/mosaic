...
class RST_HeightTest extends QueryTest with SharedSparkSessionGDAL with RST_HeightBehaviors {
    ...
    test("Testing RST_Height with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            heightBehavior(H3IndexSystem)
        }
    }
