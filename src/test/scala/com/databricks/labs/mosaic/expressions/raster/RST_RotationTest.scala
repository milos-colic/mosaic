...
class RST_RotationTest extends QueryTest with SharedSparkSessionGDAL with RST_RotationBehaviors {
    ...
    test("Testing RST_Rotation with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            rotationBehavior(H3IndexSystem)
        }
    }
