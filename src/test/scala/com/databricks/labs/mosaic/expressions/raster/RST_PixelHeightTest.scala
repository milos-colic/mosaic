...
class RST_PixelHeightTest extends QueryTest with SharedSparkSessionGDAL with RST_PixelHeightBehaviors {
    ...
    test("Testing RST_PixelHeight with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            pixelHeightBehavior(H3IndexSystem)
        }
    }
