...
class RST_ScaleYTest extends QueryTest with SharedSparkSessionGDAL with RST_ScaleYBehaviors {
    ...
    test("Testing RST_ScaleY with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            scaleYBehavior(H3IndexSystem)
        }
    }
