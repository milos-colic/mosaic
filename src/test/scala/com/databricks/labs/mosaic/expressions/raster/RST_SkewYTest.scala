...
class RST_SkewYTest extends QueryTest with SharedSparkSessionGDAL with RST_SkewYBehaviors {
    ...
    test("Testing RST_SkewY with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            skewYBehavior(H3IndexSystem)
        }
    }
