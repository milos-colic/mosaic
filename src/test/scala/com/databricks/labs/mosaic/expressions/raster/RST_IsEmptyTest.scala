...
class RST_IsEmptyTest extends QueryTest with SharedSparkSessionGDAL with RST_IsEmptyBehaviors {
    ...
    test("Testing RST_IsEmpty with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            isEmptyBehavior(H3IndexSystem)
        }
    }
