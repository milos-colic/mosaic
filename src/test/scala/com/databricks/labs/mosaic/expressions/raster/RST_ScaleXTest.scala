...
class RST_ScaleXTest extends QueryTest with SharedSparkSessionGDAL with RST_ScaleXBehaviors {
    ...
    test("Testing RST_ScaleX with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            scaleXBehavior(H3IndexSystem)
        }
    }
