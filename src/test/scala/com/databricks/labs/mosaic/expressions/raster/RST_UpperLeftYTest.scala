...
class RST_UpperLeftYTest extends QueryTest with SharedSparkSessionGDAL with RST_UpperLeftYBehaviors {
    ...
    test("Testing RST_UpperLeftY with manual GDAL registration (H3, JTS).") {
        noCodegen {
            assume(System.getProperty("os.name") == "Linux")
            upperLeftYBehavior(H3IndexSystem)
        }
    }
