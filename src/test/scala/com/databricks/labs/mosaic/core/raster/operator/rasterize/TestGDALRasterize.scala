...
class TestGDALRasterize extends SharedSparkSessionGDAL {
    ...
    val pointGeoms: Seq[JTSGeometry] = points.map(p => JTS.geometry(p, "WKT"))
    ...
    test("TestGDALRasterize: should rasterize point geometries using Z") {
        ...
        val originGeom = JTS.geometry(origin, "WKT")
        originGeom.setSpatialReference(4326)

        val raster = GDALRasterize.executeRasterize(pointGeoms, None, originGeom.asInstanceOf[JTSPoint], 3, 3, 1.0, -1.0)
