...
//        grid.pointToIndex(100, 100, 1) shouldBe 3 | resolutionMask
    ...
    test("Index to geometry") {
        ...
        // First quadrant
        val wkt0 = grid.indexToGeometry(0 | resolutionMask).toWKT
        wkt0 shouldBe "POLYGON ((0 0, 50 0, 50 50, 0 50, 0 0))"

        val wkt1 = grid.indexToGeometry(1 | resolutionMask).toWKT
        wkt1 shouldBe "POLYGON ((50 0, 100 0, 100 50, 50 50, 50 0))"

        val wkt2 = grid.indexToGeometry(2 | resolutionMask).toWKT
        wkt2 shouldBe "POLYGON ((0 50, 50 50, 50 100, 0 100, 0 50))"

        val wkt3 = grid.indexToGeometry(3 | resolutionMask).toWKT
    ...
    test("polyfill single cell") {
        ...
        grid.polyfill(geom, 1).toSet shouldBe Set(0 | resolutionMask)
        ...
        grid.polyfill(geomSmall, 1).toSet shouldBe Set()
        ...
        grid.polyfill(geomCentered, 1).toSet shouldBe Set(0 | resolutionMask)
    ...
    test("polyfill single cell with negative coordinates") {
        ...
        grid.polyfill(geom, resolution).toSet shouldBe Set(36 | resolutionMask)

        // Geometry which cell center does not fall into does not get selected
        val geomSmall = JTS.geometry("POLYGON ((0 0, 5 0, 5 5, 0 5, 0 0))", "WKT")
        grid.polyfill(geomSmall, resolution).toSet shouldBe Set()

        // Small geometry for which the cell center falls within should be detected
        val geomCentered = JTS.geometry("POLYGON ((12 12, 13 12, 13 13, 12 13, 12 12))", "WKT")
        grid.polyfill(geomCentered, resolution).toSet shouldBe Set(36 | resolutionMask)
    ...
    test("polyfill single cell with world coordinates") {
        ...
        grid.polyfill(geom, resolution).toSet shouldBe Set(34 | resolutionMask)
    ...
    test("polyfill multi cell") {
        ...
        grid.polyfill(geomMultiCell, 1).toSet shouldBe Set(
            0 | resolutionMask,
            1 | resolutionMask,
            2 | resolutionMask,
            3 | resolutionMask,
        ...
        // Small geometry that spans multiple cells should be detected
        val geomAlmostMultiCell = JTS.geometry("POLYGON ((25 25, 75 25, 75 75, 25 75, 25 25))", "WKT")
        grid.polyfill(geomAlmostMultiCell, 1).toSet shouldBe Set()
    }
    ...
    test("polyfill single DK grid") {
        ...
        assert(grid.polyfill(geom, 4).nonEmpty)
